...
class SessionManager:
    ...
    def create_session(self, prompt_file: str, output_file: str, config: Dict[str, Any]) -> str:
    ...
    def save_checkpoint(self, session_id: str, node_name: str, state: Dict[str, Any], metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.logger.info(f"Saved checkpoint {checkpoint.checkpoint_id} for session {session_id}")
    
    def load_session_state(self, session_id: str, checkpoint_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if checkpoint_id:
            checkpoint = next((cp for cp in session_info.checkpoints if cp.checkpoint_id == checkpoint_id), None)
            if not checkpoint:
            ...
            if session_dir.is_dir():
                ...
                if session_file.exists():
                    try:
                        ...
                        self.logger.warning(f"Could not load session from {session_dir}: {e}")
    ...
    def update_session_status(self, session_id: str, status: str, metadata: Optional[Dict[str, Any]] = None) -> None:
    ...
    if debug_dir.exists():
        if (debug_dir / "01_StoryElements.json").exists():
            checkpoint = SessionCheckpoint(
                ...
                timestamp=datetime.datetime.fromtimestamp((debug_dir / "01_StoryElements.json").stat().st_ctime).isoformat(),
        ...
        if (debug_dir / "02_InitialOutline.json").exists():
            checkpoint = SessionCheckpoint(
                ...
                timestamp=datetime.datetime.fromtimestamp((debug_dir / "02_InitialOutline.json").stat().st_ctime).isoformat(),
                state={"completed": True},
                metadata={"source": "migrated"}
            )
            session_info.checkpoints.append(checkpoint)
