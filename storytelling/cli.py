...
try:
    from .graph import create_graph, create_resume_graph, load_state_from_session, StoryState
...
def parse_session_arguments():
    ...
    parser = argparse.ArgumentParser(description="Storytelling - AI Story Generation System", add_help=False)
    
    # Session management commands
    parser.add_argument("--resume", type=str, help="Resume from session ID")
    parser.add_argument("--resume-from-node", type=str, help="Resume from specific node")
    parser.add_argument("--list-sessions", action="store_true", help="List available sessions")
    parser.add_argument("--session-info", type=str, help="Show information about a session")
    parser.add_argument("--migrate-session", type=str, help="Migrate existing session to new format")
...
def list_sessions(session_manager: SessionManager):
    ...
    for session in sessions:
        status_icon = {"completed": "✅", "in_progress": "🔄", "failed": "❌", "interrupted": "⏸️"}.get(session.status, "❓")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if session_args.migrate_session:
        try:
            session_info = migrate_existing_session("Logs", session_args.migrate_session)
    ...
    if session_args.resume or session_args.resume_from_node:
        ...
        try:
            ...
            if session_args.resume_from_node:
                ...
                resume_node = session_manager.get_resume_entry_point(session_args.resume)
            ...
            # Create resume graph
            graph = create_resume_graph(config, session_args.resume, session_manager, resume_node)
