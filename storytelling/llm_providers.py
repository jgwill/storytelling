...
def get_llm_from_uri(uri: str) -> BaseChatModel:
    ...
    if scheme == 'ollama':
        base_url = f"http://{host}:{port}" if host and port else "http://localhost:11434"
        ...
        if not GOOGLE_AVAILABLE:
            raise ImportError(
                f"Google Generative AI not available. "
        ...
        return PatchedOpenRouterLLM(
            model_name=model_identifier,
            **params
        )

    elif scheme == 'myflowise':
        raise NotImplementedError("The 'myflowise' provider scheme is not yet implemented.")
