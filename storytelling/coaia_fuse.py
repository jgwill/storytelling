...
class CoAiaFuseIntegrator:
    ...
    def get_prompt(self, prompt_name: str, content_only: bool = False, 
    ...
    def list_prompts(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "success",
                "raw_output": result.stdout.strip(),
                "prompts": result.stdout.strip().split('\n') if result.stdout.strip() else []
            }
    ...
    def process_dataset_for_rag(self, dataset_data: Dict[str, Any], 
        ...
        if dataset_data["status"] != "success":
            print(f"❌ Cannot process failed dataset: {dataset_data.get('error', 'Unknown error')}")
            return []
...
"""
    ...
    def process_prompt_for_rag(self, prompt_data: Dict[str, Any], 
                              output_dir: str) -> List[str]:
        ...
        if prompt_data["status"] != "success":
            print(f"❌ Cannot process failed prompt: {prompt_data.get('error', 'Unknown error')}")
            return []
