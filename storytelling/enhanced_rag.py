...
class EnhancedRAGSystem:
    ...
    def add_web_sources(self, urls: Optional[List[str]] = None, 
                       scratchpad_file: Optional[str] = None) -> int:
    ...
    def add_coaia_sources(self, dataset_names: Optional[List[str]] = None,
                         prompt_names: Optional[List[str]] = None) -> int:
        ...
        if prompt_names:
            for prompt_name in prompt_names:
                prompt_data = self.coaia_integrator.get_prompt(prompt_name, content_only=True)
    ...
    def build_unified_knowledge_base(self, embedding_model: str = "mxbai-embed-large:latest",
...
def create_enhanced_knowledge_base(scratchpad_file: str = "STORY_250907_RAG_and_More.md",
                                 existing_knowledge_dirs: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    web_count = rag_system.add_web_sources(scratchpad_file=scratchpad_file)
    
    # Add CoAiAPy sources from scratchpad
    coaia_count = rag_system.add_coaia_sources(
    ...
    if existing_knowledge_dirs:
        local_count = rag_system.add_local_sources(existing_knowledge_dirs)
    ...
    print(f"  Web content: {summary['web_files']} files from {summary['web_urls']} URLs")
    print(f"  CoAiAPy content: {summary['coaia_files']} files ({summary['coaia_datasets']} datasets, {summary['coaia_prompts']} prompts)")
    print(f"  Local content: {summary['local_files']} files from {summary['local_sources']} sources")
