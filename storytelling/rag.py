...
def get_embedding_model(model_name: str, ollama_base_url: str = "http://localhost:11434"):
    ...
    if any(ollama_model in model_lower for ollama_model in ["mxbai-embed", "nomic-embed", ":latest"]) and OllamaEmbeddings:
    ...
    elif any(ollama_indicator in model_lower for ollama_indicator in ["mxbai-embed", "nomic-embed", ":latest"]):
        if not OllamaEmbeddings:
            raise ImportError("Ollama embeddings not available. Please install langchain-ollama: pip install langchain-ollama")
        
    else:
        # Default to Ollama if available, otherwise suggest options
        if OllamaEmbeddings:
            return OllamaEmbeddings(model="mxbai-embed-large:latest", base_url=ollama_base_url)
...
def initialize_knowledge_base(knowledge_base_path: str, embedding_model: str, ollama_base_url: str = "http://localhost:11434") -> VectorStoreRetriever:
    """
    ...
    if not knowledge_base_path or not embedding_model:
        raise ValueError("Both knowledge_base_path and embedding_model must be provided")
    ...
    try:
        ...
        raise ValueError(f"Error loading documents from {knowledge_base_path}: {e}")
    
    if not documents:
        raise ValueError(f"No documents found in {knowledge_base_path}. Ensure there are .md files in the directory.")
...
def retrieve_outline_context(retriever: VectorStoreRetriever, queries: list, max_tokens: int = 1000, top_k: int = 5, similarity_threshold: float = 0.7) -> str:
    """
    ...
    for doc in all_docs[:10]:  # Limit to top 10 documents
        ...
        if total_tokens + estimated_tokens > max_tokens:
            ...
            truncated_content = doc['content'][:remaining_chars].rsplit(' ', 1)[0]  # Break at word boundary
